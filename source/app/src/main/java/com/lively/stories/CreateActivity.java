...
public class CreateActivity extends AppCompatActivity {
	...
	private HashMap<String, Object> data_input_into_firebass = new HashMap<>();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		sign_up_button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (username_edittext.getText().toString().equals("")) {
					username_edittext.setError("Name cannot be empty!");
				} else {
					if (email_edittext.getText().toString().equals("")) {
						...
						if (pass_edittext.getText().toString().equals("")) {
							pass_edittext.setError("Password cannot be empty!");
						} else {
							if (confirm_edittext.getText().toString().equals("")) {
								confirm_edittext.setError("Confirm Password cannot be empty!");
							} else {
								if (!pass_edittext.getText().toString().trim().equals(confirm_edittext.getText().toString().trim())) {
									confirm_edittext.setError("Password and Confirm Password doesn't match!");
								} else {
									fauth.createUserWithEmailAndPassword(email_edittext.getText().toString(), confirm_edittext.getText().toString().trim()).addOnCompleteListener(CreateActivity.this, _fauth_create_user_listener);
		...
		username_edittext.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (sign_up_button.getVisibility() == View.GONE) {
		...
		pass_visibility_img.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (hidden) {
					...
					pass_visibility_img.setImageResource(R.drawable.icon_visibility_off_round);
				}
				_Add("#212121", pass_visibility_img);
		...
		sign_in_txt.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (getIntent().getStringExtra("from login").equals("true")) {
					finish();
				} else {
		...
		_fauth_create_user_listener = new OnCompleteListener<AuthResult>() {
			...
			public void onComplete(Task<AuthResult> _param1) {
				...
				if (_success) {
					...
					data_input_into_firebass = new HashMap<>();
					data_input_into_firebass.put("name", username_edittext.getText().toString().trim());
					data_input_into_firebass.put("pfp", "def");
					data_input_into_firebass.put("gender", "Unknown");
					data_input_into_firebass.put("dob", "Unknown");
					data_input_into_firebass.put("bio", "Hey there! I am new to Lively! Enjoying my time here! ");
					data_input_into_firebass.put("country", "Unknown");
					data_input_into_firebass.put("verified", "no");
					data_input_into_firebass.put("date_created", new SimpleDateFormat("dd/MM/yyyy").format(calendar.getTime()));
					data_input_into_firebass.put("uid", FirebaseAuth.getInstance().getCurrentUser().getUid());
					data_input_into_firebass.put("posts_count", "0");
					data_input_into_firebass.put("token", "");
					users.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).updateChildren(data_input_into_firebass);
					startActivity(new Intent(CreateActivity.this, HomeActivity.class)); Animatoo.animateSlideLeft(CreateActivity.this);
					finish();
				} else {
	...
	private void initializeLogic() {
		...
		if (Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT) {
			final Window window = CreateActivity.this.getWindow();
	...
	public void _ui_design() {
		sign_up_txt.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_bold.ttf"), 0);
		welcome_title.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_bold.ttf"), 1);
		welcome_sub.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
		...
		already_txt.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 0);
		sign_in_txt.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
		username_edittext.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
		email_edittext.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
		pass_edittext.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
		confirm_edittext.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/poppins_medium.ttf"), 1);
